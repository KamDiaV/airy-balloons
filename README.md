...
**AIRY** — это проект сайта по продаже и доставке воздушных шаров. Сайт разворачивается на **Netlify** и использует **Serverless-функции** для обработки заказов.  
При оформлении заказа сообщение автоматически отправляется в Telegram с данными из корзины и формы.

Главная страница представляет собой "лендинг", а дополнительные страницы (каталог, страница товара, корзина и т.д.) загружаются отдельно, что позволяет лучше структурировать информацию и функционал сайта.
